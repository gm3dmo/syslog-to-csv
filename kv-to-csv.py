...
def main(args):
        ...
        with open(filename, "rb") as file:
            for line_count, line in enumerate(file.readlines()):
                ...
                try:
                    ...
                    logger.error(f"""Could not parse line number: {line_count} data: {line}""")
                    continue

                if wanted == 'core':
                    # delete the keys of parsed line we don't want
                    try:
                        raw_parsed_line = log2csv.parse_kv_pairs_two(line.rstrip('\r\n'))
                        parsed_line = {k: raw_parsed_line[k] for k in status_codes[logfile_type][wanted] if k in raw_parsed_line }
                    ...
                    if parsed_line != {}:
                        ...
                        if status_codes[logfile_type]['must'] not in parsed_line.keys():
                            skipped_count += 1
                            logger.debug(f""" i skipped line: {line_count} because: {parsed_line.keys()} does not contain: {status_codes[logfile_type]['must']} """)
                            continue
                        else:
                            pass
    ...
    logger.info(f"""Converted file: {filename} size type: {logfile_type} to CSV file {csvfile.name} size {csv_size_in_bytes} bytes or roughly {human_size_of_csv}.""")
